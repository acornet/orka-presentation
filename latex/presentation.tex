...
extended an energy profiler for Android Applications\\
specifically, focussed on hardware usage and source line level estimates}
\end{frame}
%
\iffalse
\begin{frame}
...
3 types...\\
hardware and model-based rely on \alert{expensive power measurement platforms}\\
...
explain later \alert{why important}\\
were the \alert{aims} of my project\\
After reviewing the related work...\alert{extending an existing solution}, namely Orka = best option to fulfill them.\\
}
\note[item]{
...
First, let's define tail energy
}
...
\end{column}
\end{columns}
\note{difficult to interpret: don't know what parts of the code are responsible for the drain\\
tail-energy is responsible for a \alert{significant fraction} of the drain\\~\\
...
petra = software based tool released in March\\
on a set of test applications\\
thanks to new features, generate results with orka, then compare\\~\\
...
%\item \orka{} generates a reconstructed source-code with line-level estimates
\end{itemize}
\end{block}
\note{generates a reconstructed source-code with line-level estimates}
\end{frame}
%
%
